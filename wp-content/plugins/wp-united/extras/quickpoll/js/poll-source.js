...
* @author John Wells
* 
* JavaScript for the WP-United QuickPoll widget
*/

var $wpuPoll = jQuery.noConflict();

function wpu_poll_submit(pollID, el) {
	var formData = $wpuPoll(el).parents('.wpupoll-' + pollID).find('form :first').serialize() +'&display=0';		
	return wpu_process_poll(formData, pollID);
}
...
function wpu_process_poll(data, pollID) {
	...
	$wpuPoll.post(wpuHomeURL, data, function(response) {
